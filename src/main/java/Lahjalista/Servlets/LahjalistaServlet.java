...
public class LahjalistaServlet extends HttpServlet {
    ...
    protected boolean onkoTunnistautunut(HttpServletRequest request, HttpServletResponse response) throws IOException {
        HttpSession session = request.getSession();
        Boolean tunnistautunut = (Boolean)session.getAttribute("tunnistautunut");
            
        if (tunnistautunut != null) {
            return tunnistautunut;
        }
        return false;
    }
